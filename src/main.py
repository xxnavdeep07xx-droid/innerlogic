...
def run_insta_day_pipeline(today_info):
    ...
    try:
        ...
        # Step 3: Generate background image (always — also used for cover frame + fallback)
        ...
        # Step 3b: Try fetching a mood-matched VIDEO background (huge engagement boost)
        video_bg_path = None
        try:
            from video_background_fetcher import fetch_video_background
            video_bg_path = fetch_video_background(
                category=quote.get("category", "default"),
                temp_dir=str(TEMP_DIR),
                pexels_api_key=PEXELS_API_KEY,
            )
        except Exception as e:
            logger.warning(f"   ⚠️ Video bg fetcher failed: {str(e)[:80]}")
        ...
        # Path to the cover frame (generated by video_creator inside temp_dir)
        cover_frame_path = str(TEMP_DIR / "cover_frame.png")
        ...
        if SMART_SCHEDULING:
            ...
            # Humanized offset: ±15 min random jitter to avoid exact-minute cron pattern
            jitter = random.randint(0, 15 * 60)
            logger.info(f"   ⏳ Humanized jitter: +{jitter}s before posting")
            time.sleep(jitter)
        else:
        ...
        if track_info:
            ...
            result = _upload_without_music(cl, video_path, caption, cover_frame_path)
        ...
        # Step 9: Anti-bot engagement — like a few reels to look human
        try:
            logger.info("👀 Step 9: Anti-bot engagement (browsing & liking)...")
            from instagram import engage_after_post
            engage_after_post(cl, max_likes=4)
        except Exception as e:
            logger.warning(f"   ⚠️  Engagement step skipped: {str(e)[:80]}")
...
def _upload_without_music(cl, video_path, caption, cover_frame_path=None):
    """Upload reel without Instagram native music (fallback)."""
    from instagram import upload_reel
    return upload_reel(cl, video_path, caption, custom_thumbnail=cover_frame_path)
